...
class Authentication:
    ...
    def create_user(self, name: str, email: str, password: str, address: str, credit: float = 0) -> User:

        session = SessionLocal()
        try:
            existing_basic_user = session.query(BasicUserDB).filter(BasicUserDB.email == email).first()
    ...
    def set_new_password(self, curr_basic_user : Union[User, Manager, None], new_password: str) -> None:
        """Update user's/manager's password."""
        session = SessionLocal()
        try:
            basic_user_db = session.query(BasicUserDB).filter(BasicUserDB.email == curr_basic_user.email).first()
