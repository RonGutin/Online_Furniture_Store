...
def test_complete_purchase_workflow():
    ...
    with patch("app.data.DbConnection.SessionLocal") as mock_session_factory:
        ...
        with patch.object(DiningTable, '_get_info_furniture_by_key', 
        ...
        with patch.object(dining_table, 'check_availability', return_value=True):
            ...
            with patch.object(dining_table, 'check_availability', return_value=False):
                assert not dining_table.check_availability(100), "Should not be available in large quantity"
        ...
        with patch.object(dining_table, 'check_availability', return_value=True):
            with patch.object(dining_table, 'Print_matching_product_advertisement', 
                ...
                assert user.cart.items[0][0] == dining_table, "Cart item should be the dining table"
        ...
        with patch.object(user.cart, 'get_coupon_discount_and_id', return_value=(10, 1)):
            # Test the actual apply_discount method
            discounted_total = user.cart.apply_discount(10)
            expected_discount = 400.0 * 0.9  # 10% off
            assert discounted_total == expected_discount, f"Expected discounted price to be {expected_discount}, got {discounted_total}"
        
        # 7. Test checkout process - use the real checkout and validate_credit_card, but mock other dependencies
        with patch.object(dining_table, 'check_availability', return_value=True):
            with patch('app.models.inventory.Inventory.update_amount_in_inventory') as mock_update_inventory:
                ...
                with patch.object(Order, '_save_to_db'):
                    ...
                    assert args[0] == dining_table, "Should update inventory for dining table"
                    assert args[1] == 2, "Should update quantity of 2"
                    assert kwargs["sign"] == False, "Should reduce inventory"
                    
                    # Verify order was created
                    assert len(user._orders) == 1, "Order should be created"
                    order = user._orders[0]
                    assert order.get_user_mail() == "test@example.com", "Order should have correct email"
                    
                    # Verify cart was emptied
                    assert len(user.cart.items) == 0, "Cart should be empty after checkout"
        ...
        assert "no items in the cart" in msg.lower(), f"Expected error about empty cart, got: {msg}"
        ...
        with patch.object(dining_table, 'check_availability', return_value=False):
            ...
            assert "not enough" in msg.lower(), f"Expected error about availability, got: {msg}"
        ...
        with patch.object(dining_table, 'check_availability', return_value=True):
            ...
            assert "invalid credit card" in msg.lower(), f"Expected error about invalid credit card, got: {msg}"