...
class TestFurnitureFactory(unittest.TestCase):
    ...
    def test_invalid_furniture_type(self):
        with self.assertRaises(ValueError):
            self.factory.create_furniture("SOFA", color="brown", material="wood")

    def test_missing_parameter_for_table(self):
        with self.assertRaises(TypeError):
            self.factory.create_furniture("DINING_TABLE", color="brown")

    def test_missing_parameter_for_chair(self):
        with self.assertRaises(TypeError):
            self.factory.create_furniture(
                "WORK_CHAIR", color="black", is_adjustable=True
            )

    def test_invalid_color_type(self):
        with self.assertRaises(TypeError):
            self.factory.create_furniture("COFFEE_TABLE", color=123, material="glass")

    def test_invalid_material_type(self):
        with self.assertRaises(TypeError):
            self.factory.create_furniture("WORK_DESK", color="black", material=None)
